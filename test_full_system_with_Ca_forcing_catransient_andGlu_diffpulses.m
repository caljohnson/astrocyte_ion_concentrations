...
for jj=1:2
if jj==1
    %with both Glu and Ca forcing
%     Ca_force = @(t) 0.*t + 1e-1.*(t-0).*(t>=0 & t<5) + ...
%         (5e-1 - 1e-1.*(t-5)).*(t>=5 & t<10) ; %one peak
%     Glu_force = @(t) 250.*(0.*t + 1e-1.*(t-0).*(t>=0 & t<0.5) + ...
%         (5e-1 - 1e-1.*(t-0.5)).*(t>=0.5 & t<1.0)) ; %one peak

     Ca_force = @(t) 0.*t + 1e-1.*(t-0).*(t>=0 & t<5) + ...
        (5e-1 - 1e-1.*(t-5)).*(t>=5 & t<10) +...
        0.5e-1.*(t-25).*(t>=25 & t<30) + ...
        (2.5e-1 - 0.5e-1.*(t-30)).*(t>=30 & t<35); %two peak
    
    Glu_force = @(t) 25.*(0.*t + 1e-1.*(t-0).*(t>=0 & t<0.5) + ...
        (5e-1 - 1e-1.*(t-0.5)).*(t>=0.5 & t<1.0) +...
        0.5e-1.*(t-25).*(t>=25 & t<25.5) + ...
        (2.5e-1 - 0.5e-1.*(t-25.5)).*(t>=25.5 & t<26)); %two peak
% elseif jj==3
...
elseif jj==2
    %with only Glu  forcing
    Ca_force = @(t) 0.*t; %no Ca forcing
%     Glu_force = @(t) 250.*(0.*t + 1e-1.*(t-0).*(t>=0 & t<0.5) + ...
%         (5e-1 - 1e-1.*(t-0.5)).*(t>=0.5 & t<0.10)) ; %one peak
...
[t,X] = ode15s(@(t,X) system_eqns_with_Glu_signal(t,X,Glu_force,Ca_force), tspan, X0,options);
dt = 1e-1;
...
% save('Na_K_outs_Glu_diffpulses.mat','ts','Na_outs', 'K_outs');
end
% figure(5);
