...
lgd = legend('Matsuoka & Hilgemann 1992 Fig9B data','Egger & Niggli 1999 model',...
    'Kimura et al. 1987 model', 'Gall & Susa 1999 model',...
    'Flanagan et al. 2018 model');
lgd.Location = 'northwest';


line([-150,100],[0,0],'LineStyle','--', 'Color', 'k','HandleVisibility','off');
line([0,0],[-100,100],'LineStyle','--', 'Color', 'k','HandleVisibility','off');


title('[Na^+]_i = 10 mM, [Ca^{2+}]_i = 0.1 \muM');
xlim([-150,100]); ylim([-40,60])
...
lgd = legend('Matsuoka & Hilgemann 1992 Fig9A data','Egger & Niggli 1999 model',...
    'Kimura et al. 1987 model', 'Gall & Susa 1999 model',...
    'Flanagan et al. 2018 model');
lgd.Location = 'northwest';
...
[Vm_data, I_data] = csvimport('data/matsuoka_hilgemann_1992_fig9a_50.csv',...
                    'columns', [1, 2] ,'noHeader', true);              
Na_in = 50;%mM          
figure(2); clf;
plot(Vm_data,I_data,'o','LineWidth',4,'MarkerSize',10); hold on
...
lgd = legend('Matsuoka & Hilgemann 1992 Fig9A data','Egger & Niggli 1999 model',...
    'Kimura et al. 1987 model', 'Gall & Susa 1999 model',...
    'Flanagan et al. 2018 model');%, 'Ca_i Hill + Na_i Hill', 'Linear');
lgd.Location = 'northwest';
...
[Vm_data, I_data] = csvimport('data/matsuoka_hilgemann_1992_fig9a_20.csv',...
                    'columns', [1, 2] ,'noHeader', true);              
Na_in = 20;%mM          
figure(3); clf;
plot(Vm_data,I_data,'o','LineWidth',4,'MarkerSize',10); hold on
...
[Vm_data, I_data] = csvimport('data/matsuoka_hilgemann_1992_fig9a_5.csv',...
                    'columns', [1, 2] ,'noHeader', true);              
Na_in = 5;%mM          
figure(4); clf;
plot(Vm_data,I_data,'o','LineWidth',4,'MarkerSize',10); hold on
...
V_NaCa = (R*T/F)*(3*log(Na_out/Na_in)-log(Ca_out/Ca_in))*1e3; %mV
I_NaCa_gall_susa = @(x,Vm) x*(Ca_in.^nH)./(Ca_in.^nH + K_H^nH).*(Vm-V_NaCa);
% gall_susa_fit2 = lsqcurvefit(I_NaCa_gall_susa,-0.07,Vm_data,I_data);
plot(Vm,I_NaCa_gall_susa(gall_susa_fit,Vm),'--','LineWidth',4);
...
line([-150,100],[0,0],'LineStyle','--', 'Color', 'k','HandleVisibility','off');
line([0,0],[-100,200],'LineStyle','--', 'Color', 'k','HandleVisibility','off');


title('[Na^+]_i = 100 mM, [Ca^{2+}]_i = 0.1 \muM');
xlim([-150,100]); ylim([-50,120])
