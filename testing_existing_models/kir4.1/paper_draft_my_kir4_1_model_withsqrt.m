...
% based on I-V data
% with square-root of external potassium dependence like older models
...
I_K1 = @(x,Vm) x(1).*(Vm-VKA1-x(2)).*sqrt(Ke)./(1+exp((Vm-VKA1-x(2))./x(3))); %units nA
...
I_K1 = @(x,Vm) x(1).*(Vm-VKA4-x(2)).*sqrt(Ke)./(1+exp((Vm-VKA4-x(2))./x(3))); %units nA
...
C = 0.05; %pF
I_K2 = @(x,Vm) x(1)*(Vm-VKA2-x(2)).*sqrt(Ke)./(1+exp((Vm-VKA2-x(2))./x(3))); %units nA
...
I_K3 = @(x,Vm) x(1)*(Vm-VKA3-x(2)).*sqrt(Ke)./(1+exp((Vm-VKA3-x(2))./x(3))); %units nA
...
saveas(fig,'figs/paper_figs/fig_model_fits_all_withsqrt.png');
...
line([-165,60],[0,0],'LineStyle','--', 'Color', 'k','HandleVisibility','off');
line([0,0],[-150,100],'LineStyle','--', 'Color', 'k','HandleVisibility','off');
saveas(fig,'figs/paper_figs/fig_model_fits_sicca_withsqrt.png');
...
line([-165,60],[0,0],'LineStyle','--', 'Color', 'k','HandleVisibility','off');
line([0,0],[-150,100],'LineStyle','--', 'Color', 'k','HandleVisibility','off');
saveas(fig,'figs/paper_figs/fig_model_fits_chai_withsqrt.png');
...
set(gca,'XTick',1:6,'xticklabel',{'1','2','3','4','5','6'},'FontSize',20)
saveas(fig,'figs/paper_figs/fig_model_params_withsqrt.png');
...
subplot(2,2,1);
for ii=1:size(Kes,2)
   I_K = @(x,Vm) x(1)*(Vm-VKAs(ii)-x(2)).*sqrt(Kes(ii))./(1+exp((Vm-VKAs(ii)-x(2))./x(3))); %units nA
...
saveas(fig,'figs/paper_figs/fig_model_Ke_changes_withsqrt.png');
...
line([0,0],[-150,100],'LineStyle','--', 'Color', 'k','HandleVisibility','off');
title('Model 5');
saveas(fig,'figs/paper_figs/fig_model_Ke_chances_with_Vr_withsqrt.png');
...
legend('Nernst potential','V_{rest}, model 1',...
    ...
    'Location','southeast');
set(gca,'FontSize',20);
saveas(fig,'figs/paper_figs/fig_model_Vr_changes_withsqrt.png');
