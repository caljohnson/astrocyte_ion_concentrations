...
kglu = 1e-1;
nglu = 2;
% I_EAAT2 = @(x,Vm) x(1).*(Vm-V_rev).*exp(x(2).*(Vm-V_rev)).*(log10(Glu_out).^nglu./(kglu+log10(Glu_out).^nglu));
I_EAAT2 = @(x,Vm) x(1).*(Vm-V_rev).*exp(x(2).*(Vm-V_rev)).*(Glu_out.^nglu./(kglu.^nglu+Glu_out.^nglu));
...
V_rev = (R*T/(2*F)).*log((Na_out/Na_in)^3*(K_in/K_out)*(H_out/H_in).*(Glu_out./Glu_in))*1e3; %mV
V_r_data = (R*T/(2*F)).*log((Na_out/Na_in)^3*(K_in/K_out)*(H_out/H_in).*(Glu_out_data./Glu_in))*1e3; %mV
V_r_fn = @(Glu) (R*T/(2*F)).*log((Na_out/Na_in)^3*(K_in/K_out)*(H_out/H_in).*(Glu./Glu_in))*1e3; %mV
%remake model functions with new V_rev
I_EAAT2_flan = @(x,Vm,V_r) -x(1).*exp(x(2).*(Vm-V_r));
I_EAAT2 = @(x,Vm,Glu) x(1).*(Vm-V_rev).*exp(x(2).*(Vm-V_r_fn(Glu))).*(Glu.^nglu./(kglu.^nglu+Glu.^nglu));
% I_EAAT2 = @(x,Glu) x(1).*(Vm-V_r_fn(Glu)).*exp(x(2).*(Vm-V_r_fn(Glu))).*(Glu.^nglu./(x(3)+Glu.^nglu));
% I_EAAT2 = @(x,Glu) (log10(Glu).^nglu./(x(1)+log10(Glu).^nglu));
% I_EAAT2 = @(x,Glu) (Glu.^nglu./(x(1)+Glu.^nglu));
% my_fit = lsqcurvefit(I_EAAT2,[1,10,0.1],Glu_out_data,I_Glu_data);
% my_fit = lsqcurvefit(I_EAAT2,0.002,Glu_out_data,I_Glu_data);

%plot
figure(3);clf;
semilogx(Glu_out, -I_EAAT2_flan(flanagan_fit,Vm,V_rev)./max(abs(I_EAAT2_flan(flanagan_fit,Vm,V_rev))),...
    'b-','LineWidth',3); hold on
% semilogx(Glu_out, -I_EAAT2(my_fit,Vm,V_rev)./max(abs(I_EAAT2(my_fit,Vm,V_rev))), 'r-','LineWidth',3);
semilogx(Glu_out, -I_EAAT2(my_fit,Vm,Glu_out)./max(abs(I_EAAT2(my_fit,Vm,Glu_out))), 'r-','LineWidth',3);
% semilogx(Glu_out, I_EAAT2(my_fit,Glu_out), 'r-','LineWidth',3);
...
I_EAAT2_flan = @(x,Vm) -x(1).*exp(x(2).*(Vm-V_rev));
%my model
I_EAAT2 = @(x,Vm) x(1).*(Vm-V_rev).*exp(x(2).*(Vm-V_rev));
...
set(gca,'FontSize',20);
xlim([-125,140]); ylim([-100,0]);
plot(Vm_data2, I_data2, 'b^','MarkerSize',10,'LineWidth',2)
...
%remake model functions with new V_rev
I_EAAT2_flan = @(x,Vm,V_r) -x(1).*exp(x(2).*(Vm-V_r));
I_EAAT2 = @(x,Vm,V_r) x(1).*(Vm-V_r).*exp(x(2).*(Vm-V_r));
%plot
figure(4);clf;
semilogx(Glu_out, -I_EAAT2_flan(flanagan_fit2,Vm,V_rev)./max(abs(I_EAAT2_flan(flanagan_fit2,Vm,V_rev))),...
    'b-','LineWidth',3); hold on
semilogx(Glu_out, -I_EAAT2(my_fit2,Vm,V_rev)./max(abs(I_EAAT2(my_fit2,Vm,V_rev))), 'r-','LineWidth',3);
xlabel('extracellular Glutamate concentration, mM'); ylabel('current, normalized');
set(gca,'FontSize',20);
semilogx(Glu_out_data, I_Glu_data, 'b^','MarkerSize',10,'LineWidth',2)
semilogx(Glu_out_data2, I_Glu_data2, 'm^','MarkerSize',10,'LineWidth',2)
legend('Flanagan et al., 2018 model', 'my model', ...
    'Bergles and Jahr, 1997 data, steady-state', 'Bergles and Jahr, 1997 data, peak')
...
figure(6);clf;
plot(Glu_out, I_EAAT2_flan(flanagan_fit,Vm,V_rev),...
    'b-','LineWidth',3); hold on
plot(Glu_out, I_EAAT2(my_fit,Vm,V_rev), 'r-','LineWidth',3);
xlabel('extracellular Glutamate concentration, mM'); ylabel('current, set to max Flanagan');
set(gca,'FontSize',20);
plot(Glu_out_data, I_Glu_data*min(I_EAAT2_flan(flanagan_fit,Vm,V_rev)), 'b^','MarkerSize',10,'LineWidth',2)
plot(Glu_out_data2, I_Glu_data2*min(I_EAAT2_flan(flanagan_fit,Vm,V_rev)), 'm^','MarkerSize',10,'LineWidth',2)
legend('Flanagan et al., 2018 model', 'my model', ...
    'Bergles and Jahr, 1997 data, steady-state', 'Bergles and Jahr, 1997 data, peak')
...
plot(Glu_out, I_EAAT2_flan(flanagan_fit,Vm,V_rev),...
    'b-','LineWidth',3); hold on
plot(Glu_out, I_EAAT2(my_fit,Vm,V_rev), 'r-','LineWidth',3);
xlabel('extracellular Glutamate concentration, mM'); ylabel('current, set to max mine');
set(gca,'FontSize',20);
plot(Glu_out_data, I_Glu_data*min(I_EAAT2(my_fit,Vm,V_rev)), 'b^','MarkerSize',10,'LineWidth',2)
plot(Glu_out_data2, I_Glu_data2*min(I_EAAT2(my_fit,Vm,V_rev)), 'm^','MarkerSize',10,'LineWidth',2)
legend('Flanagan et al., 2018 model', 'my model', ...
    'Bergles and Jahr, 1997 data, steady-state', 'Bergles and Jahr, 1997 data, peak')
...
norm_factor = I_EAAT2_flan(flanagan_fit,Vm,V_rev_for_norm_factor)./I_Glu_data(3);
...
xlabel('extracellular Glutamate concentration, mM'); ylabel('current, set to max Flanagan');
set(gca,'FontSize',20);
plot(Glu_out_data, I_Glu_data*norm_factor, 'b^','MarkerSize',10,'LineWidth',2)
