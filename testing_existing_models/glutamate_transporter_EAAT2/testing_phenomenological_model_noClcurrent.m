...
% but without the chloride current (inspired by lack of chloride-dependence
% shown in Wadiche et al. 1995b Fig3b.
...
I_EAAT2 = @(x,Vm) (Na_out^2.3./(kNa^2.3+Na_out^2.3))*(H_out./(kH+H_out))*...
    (K_in./(kK+K_in))*(Glu_out./(kglu+Glu_out))*(-x(1)*exp(-x(2)*Vm));


%my fit
my_fit = lsqcurvefit(I_EAAT2,[16,0.02],Vm_data,I_data);
